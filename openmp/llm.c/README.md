...
All versions (sequential, fork-join, task-based, distributed-task-based) are all fused into the distributed-task-based branch, so I recommand to first go on this branch.

### Install

To run the model on MN5, you first have to set up your local environment. For this, run
```bash
pip3 install -r requirements.txt
python3 dev/data/tinyshakespeare.py
python3 dev/data/tinystories.py
OMP_NUM_THREADS=8 MKL_NUM_THREADS=8 python3 train_gpt2.py
```
### Compile

Before compiling you have to load the following modules
```
module load ompss-2 # For all versions
module load tampi # For mpi version
```
Mind that, in general, all versions get slightly better results when using ompss-2/git. Moreover, it is highly recommended to use the tampi/git module as it increases significantly the performances.

To compile, you have two choices :
```
# Compilation script :
./compile.sh -h

# Or pass by the Makefile :
make train_gpt2 # Sequential
make train_gpt2_openmp
make train_gpt2_openmpv
make train_gpt2_ompss
make train_gpt2_ompss-mpi
make train_gpt2_openmpv-mpi
```

The bash script allows you to set up batch size, sequence size and other values at compile time, without editing the code.


### Run

To run you script, see :
```
./launcher.sh -h

# For example, you can run :
./compile.sh -v ompss -B 4 -T 64 -t # Compile OmpSs-2 version with B=4, T=64 and enabling taskiter; 
srun -c 56 -n 1 ./launcher.sh -v ompss -c 56  # Run OmpSs-2 version with 56 CPUs

# Or also
./compile.sh -v ompss-mpi -B 16 -T 1024 -t -i 40 -V -I # OmpSs-2 with MPI version, B=16, T=1024, enable taskiter, 40 iterations, disbale model validation and inference
./launcher -v ompss-mpi -A bsc15 -q gp_debug -i -n 4 -c 56 # OmpSs-2 with MPI version, slurm accounts and queue, enable instrumentation, 4 ranks, 56 CPUs/rank
```

By default, model inference and validation are enabled, however it is recommended to disable those for benchmarking (-V and -I option at compile time). 
The 'Ompss - MPI' & 'OpenMP/nOS-V - MPI' versions are the only ones that allows you to specify a slurm account and queue, as these are the only ones to use a srun command.
All scripts (compile or launcher) are printing out the executed command, so you can retrieve it there if you want to run directly the commands by yourself.

### Enable instrumentation

OVNI instrumentation is available for OpenMP/nOS-V, OmpSs-2 and MPI versions. To run with instrumentation, execute with :
```
# OmpSs-2 version with 56 CPUs
srun -c 56 -n 1 ./launcher.sh -v ompss -i -c 56 # -i for instrumentation 
```
