...
// Using the most up-to-date test from Karpathy, and updated it to work with
// JLESC.
...
train_gpt2.py runs the same model on the same inputs, and then saves its results to gpt2_124M_debug_state.bin

This test scripts reproduces the steps done in train_gpt2.py with the functions in train_gpt2.c.
It then checks that  it gets the same results as PyTorch - within a specified tolerance. 
...
int main(int argc, char *argv[]) 
{
    ...
    printf("B=%d, T=%d, rank_B=%d, rank_T=%d\n", B, T, mpi_get_rank_B(worker), mpi_get_rank_T(worker));
    ...
    #pragma omp single
    {
        for (int step = 0; step < 10; step++) 
        {
            ...
            gpt2_forward(&model, worker, x, y, mpi_get_rank_B(worker), mpi_get_rank_T(worker), dep_handler);
            ...
            {
                gpt2_update(&model, 1e-4f, 0.9f, 0.999f, 1e-8f, 0.01f, &step_num);
            }
            
            // WHile gpt2_update is important for the model and has its own tasks,
            // it is also
            #pragma omp taskwait
