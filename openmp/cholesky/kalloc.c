/*
 * kalloc.c - implementation of the host memory allocator abstraction.
 */
#include "kalloc.h"
...
# else
#  include <omp.h>
#  define CH_ALLOC_OMP_PINNED 1
# endif
#endif

#if defined(CH_ALLOC_OMP_PINNED)
/* Lazily-created pinned allocator (all allocations happen before the parallel
 * region, so no synchronization is needed here). */
static omp_allocator_handle_t ch_pinned = omp_null_allocator;

static void ch_pinned_init(void)
{
    if (ch_pinned != omp_null_allocator) return;
    omp_alloctrait_t traits[1];
    traits[0].key   = omp_atk_pinned;
    traits[0].value = omp_atv_true;
    ch_pinned = omp_init_allocator(omp_default_mem_space, 1, traits);
    if (ch_pinned == omp_null_allocator)
...
# else
    ch_pinned_init();
    if (ch_pinned != omp_null_allocator) return omp_alloc(bytes, ch_pinned);
    return malloc(bytes);
# endif
#else
    return malloc(bytes);
...
# elif defined(ALLOC_HIP)
    hipHostFree(ptr);
# else
    if (ch_pinned != omp_null_allocator) omp_free(ptr, ch_pinned);
    else free(ptr);
# endif
#else
    free(ptr);
