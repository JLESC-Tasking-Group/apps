...
/** https://en.cppreference.com/w/cpp/named_req/Allocator */
template <class T>
struct MallocatorTarget
{
  typedef T value_type;

  MallocatorTarget () = default;
  template <class U> constexpr MallocatorTarget (const MallocatorTarget <U>&) noexcept {}

  [[nodiscard]] T* allocate(std::size_t n) {
    T * p = AllocateForTarget<T>(n);
    if (p) return p;
    throw std::bad_alloc();
  }
...
template <class T, class U>
bool operator==(const MallocatorTarget <T>&, const MallocatorTarget <U>&) { return true; }
template <class T, class U>
bool operator!=(const MallocatorTarget <T>&, const MallocatorTarget <U>&) { return false; }
...
class Domain {
  ...
  // private:
   ...
   std::vector<Real_t, MallocatorTarget<Real_t>> m_vdov ;  /* volume derivative over volume */

   std::vector<Real_t, MallocatorTarget<Real_t>> m_arealg ;  /* characteristic length of an element */

   std::vector<Real_t, MallocatorTarget<Real_t>> m_ss ;      /* "sound speed" */
