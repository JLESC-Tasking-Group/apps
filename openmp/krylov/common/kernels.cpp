               ...
               idx_t nnz, const real_t *x, real_t *y, char *y_tok, const Tiling *tl)
{
    ...
    for (int t1 = 0; t1 < NTB1; t1++) {
        ...
        for (int t2 = 0; t2 < T2; t2++) {
            ...
            OMP_TASK(firstprivate(begin, end) DEPEND_MULTI(in, (b=0:NTB1), x[b * BS]) DEPEND(out, y_tok[t1 * T2 + t2]))
            {
...
void task_scal(const Tiling *tl, const real_t *s, real_t *y)
{
    const idx_t n  = tl->n;
    const idx_t bs = USE_TARGET ? (idx_t) 1 : tl->BS;
    OMP_TARGET_LOOP_TASK(DEPEND(in, s[0]) DEPEND(inout, y[0]) MAP(present, alloc: s[0:1], y[0:n]))
    for (idx_t blk = 0; blk < n; blk += bs) {
        OMP_TASK(firstprivate(blk, bs, n) DEPEND(in, s[0]) DEPEND(inout, y[blk]))
        {
            const real_t c = s[0];
            const idx_t end = KR_MIN(blk + bs, n);
...
void task_scal_copy(const Tiling *tl, const real_t *s, const real_t *x, real_t *y)
{
    const idx_t n  = tl->n;
    const idx_t bs = USE_TARGET ? (idx_t) 1 : tl->BS;
    OMP_TARGET_LOOP_TASK(DEPEND(in, s[0], x[0]) DEPEND(out, y[0]) MAP(present, alloc: s[0:1], x[0:n], y[0:n]))
    for (idx_t blk = 0; blk < n; blk += bs) {
        OMP_TASK(firstprivate(blk, bs, n) DEPEND(in, s[0], x[blk]) DEPEND(out, y[blk]))
        {
            const real_t c = s[0];
            const idx_t end = KR_MIN(blk + bs, n);
...
void task_copy_spmv(const Tiling *tl, const real_t *ys, char *ys_tok, real_t *y)
{
    ...
    (void) ys_tok;
#if USE_TARGET
    OMP_TARGET_LOOP_TASK(DEPEND(in, ys[0]) DEPEND(out, y[0]) MAP(present, alloc: ys[0:n], y[0:n]))
    ...
    for (int t1 = 0; t1 < NTB1; t1++) {
        const idx_t begin = (idx_t) t1 * BS;
        const idx_t end   = KR_MIN(begin + BS, n);
        OMP_TASK(firstprivate(begin, end) DEPEND_MULTI(in, (t2=0:T2), ys_tok[t1 * T2 + t2]) DEPEND(out, y[begin]))
        {
...
void task_vmul_spmv(const Tiling *tl, const real_t *d, const real_t *ys, char *ys_tok, real_t *y)
{
    ...
    (void) ys_tok;
#if USE_TARGET
    OMP_TARGET_LOOP_TASK(DEPEND(in, d[0], ys[0]) DEPEND(out, y[0]) MAP(present, alloc: d[0:n], ys[0:n], y[0:n]))
    ...
    for (int t1 = 0; t1 < NTB1; t1++) {
        const idx_t begin = (idx_t) t1 * BS;
        const idx_t end   = KR_MIN(begin + BS, n);
        OMP_TASK(firstprivate(begin, end) DEPEND(in, d[begin]) DEPEND_MULTI(in, (t2=0:T2), ys_tok[t1 * T2 + t2]) DEPEND(out, y[begin]))
        {
...
void task_axpy_spmv(const Tiling *tl, const real_t *s, real_t sign, const real_t *ys, char *ys_tok, real_t *y)
{
    ...
    (void) ys_tok;
#if USE_TARGET
    OMP_TARGET_LOOP_TASK(DEPEND(in, s[0], ys[0]) DEPEND(inout, y[0]) MAP(present, alloc: s[0:1], ys[0:n], y[0:n]))
    ...
    for (int t1 = 0; t1 < NTB1; t1++) {
        const idx_t begin = (idx_t) t1 * BS;
        const idx_t end   = KR_MIN(begin + BS, n);
        OMP_TASK(firstprivate(begin, end, sign) DEPEND(in, s[0]) DEPEND_MULTI(in, (t2=0:T2), ys_tok[t1 * T2 + t2]) DEPEND(inout, y[begin]))
        {
            const real_t c = sign * s[0];
...
void task_xpby_spmv(const Tiling *tl, const real_t *ys, char *ys_tok, const real_t *s, real_t *y)
{
    ...
    (void) ys_tok;
#if USE_TARGET
    OMP_TARGET_LOOP_TASK(DEPEND(in, ys[0], s[0]) DEPEND(inout, y[0]) MAP(present, alloc: ys[0:n], s[0:1], y[0:n]))
    ...
    for (int t1 = 0; t1 < NTB1; t1++) {
        const idx_t begin = (idx_t) t1 * BS;
        const idx_t end   = KR_MIN(begin + BS, n);
        OMP_TASK(firstprivate(begin, end) DEPEND(in, s[0]) DEPEND_MULTI(in, (t2=0:T2), ys_tok[t1 * T2 + t2]) DEPEND(inout, y[begin]))
        {
            const real_t c = s[0];
...
void task_dot_spmv(const Tiling *tl, const real_t *a, const real_t *ys, char *ys_tok, real_t *part, real_t *result)
{
    ...
    for (int t1 = 0; t1 < NTB1; t1++) {
        const idx_t begin = (idx_t) t1 * BS;
        const idx_t end   = KR_MIN(begin + BS, n);
        OMP_TASK(firstprivate(begin, end, t1) DEPEND(in, a[begin]) DEPEND_MULTI(in, (t2=0:T2), ys_tok[t1 * T2 + t2]) DEPEND(out, part[t1]))
        {
            real_t s = (real_t) 0.0;
