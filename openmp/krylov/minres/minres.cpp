                         ...
                         const KrylovParams *prm, KrylovStats *st)
{
    ...
    #pragma omp single
    {
                       ...
                       [&] (size_t inst, size_t done)
        {
             ...
             * Anchored on phibar (MINRES tracks ||r|| = phibar by recurrence);
            ...
            if (print_dbg) {
                OMP_TARGET_UPDATE(from(phibar[0:1]) EPILOGUE_NOWAIT DEPEND(inout, phibar[0]))
            }
            EPILOGUE_TASK(DEFAULT_NONE firstprivate(inst, unroll, phibar, print_dbg, st)
